...
function fmtNum(n) {
  ...
  if (s.indexOf('e') !== -1) s = Number(n).toFixed(10).replace(/\.?0+$/, '');
  return s || '0';
}
  ...
  return (
    <div className="app-main-offset p-3 p-md-4" style={{ maxWidth: 1100, margin: '0 auto' }}>
      <div className="d-flex align-items-center gap-2 mb-3">
        <Link to={`/invoice/${id}`} className="btn btn-outline-secondary btn-sm">
          ← View
        </Link>
        <h4 className="mb-0">Edit invoice</h4>
      </div>
      ...
      <form onSubmit={onSubmit} className="card border-0 shadow-sm">
        <div className="card-body">
          <div className="d-flex gap-2 mb-3">
            <button
              type="button"
              className={`btn btn-sm ${!isGst ? 'btn-primary' : 'btn-outline-primary'}`}
          ...
          <div className="row g-3">
            <div className="col-md-6">
              ...
              <input
                ...
                required
              />
              <label className="form-label small fw-bold mt-2">Email</label>
              <input
                type="email"
                className="form-control"
                value={clientEmail}
              ...
              />
              <label className="form-label small fw-bold mt-2">Phone</label>
              <input
                className="form-control"
                value={clientPhone}
            ...
            <div className="col-md-6">
              ...
              <input
                ...
                required
              />
              <label className="form-label small fw-bold mt-2">Tax / GST rate %</label>
              <input
                type="number"
                className="form-control"
                step="any"
          ...
          <table className="table table-sm">
            ...
            <tbody>
              {items.map((row, i) => {
                const amt = (parseFloat(row.quantity) || 0) * (parseFloat(row.price) || 0);
                return (
                  <tr key={i}>
                    <td>
                      <input
          ...
          <div className="text-end small mb-3">
            Subtotal ₹{fmtNum(totals.subtotal)} · Tax ₹{fmtNum(totals.taxAmount)} ·{' '}
            <strong>Total ₹{fmtNum(totals.total)}</strong>
          </div>
